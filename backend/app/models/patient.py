...
class Patient(CreatedOnlyModel):
    ...
    address = Column(Text)
    date_of_visit = Column(Date, index=True)
    total_visit = Column(Integer, default=1)
    notes = Column(Text)
    total_amount = Column(Numeric(10, 2), default=0)
    payment_status = Column(Enum("paid", "partial", "pending", name="payment_status_enum"), default="pending", index=True)
    ...
    # Composite index for the most common query: filter by clinic + lookup by patient_uid
    __table_args__ = (
        Index("ix_patients_clinic_uid", "clinic_id", "patient_uid"),
    )
