...
def list_subscriptions(db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    return get_subscriptions(db)


@router.post("", response_model=SubscriptionResponse, status_code=status.HTTP_201_CREATED)
def add_subscription(data: SubscriptionCreate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    return create_subscription(db, data)


@router.put("/{subscription_id}/renew", response_model=SubscriptionResponse)
def renew(subscription_id: int, data: SubscriptionRenew, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
