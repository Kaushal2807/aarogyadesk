...
def add_user(data: UserCreate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    if data.user_type == "clinic" and not data.clinic_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="clinic_id is required for clinic users")
...
def edit_user(user_id: int, data: UserUpdate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
    ...
    # Determine effective user_type after update
    effective_user_type = update_data.get("user_type", user.user_type)
    effective_clinic_id = update_data.get("clinic_id", user.clinic_id)
    if effective_user_type == "clinic" and not effective_clinic_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="clinic_id is required for clinic users")
